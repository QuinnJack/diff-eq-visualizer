...
function setup() {
  ...
  for (var i = 0; i < stars; i++) {
    points.push(
      new Point(
        (random() - 1 / 2) * width * zoom_scale,
        (random() - 1 / 2) * height * zoom_scale
      )
    );
...
function mouseWheel(event) {
  ...
  document.getElementById("zoom").innerHTML = Number.parseFloat(
    zoom_scale * 100
  ).toExponential(2); //str(100*zoom_scale).substring(0,7);
}
...
function draw() {
  ...
  for (var i = 0; i < stars; i++) {
    ...
    speed = sqrt(
      h * points[i].py * h * points[i].py + h * points[i].px * h * points[i].px
    );

    stroke(
      150 - (100 * xspeed) / speed,
      150 + speed * 60,
      150 + (100 * yspeed) / speed
    ); // color
    line(
      points[i].xpos / zoom_scale + x_off + width / 2,
      (-1 * points[i].ypos) / zoom_scale + y_off + height / 2,
      points[i].px / zoom_scale + x_off + width / 2,
      (-1 * points[i].py) / zoom_scale + y_off + height / 2
    );
    ...
    if (
      points[i].ypos / zoom_scale < -height / 2 ||
      points[i].xpos / zoom_scale < -width / 2 ||
      points[i].xpos / zoom_scale > width / 2 ||
      points[i].ypos / zoom_scale > height / 2 ||
      points[i].life < 0
    ) {
      points[i].xpos =
        (random() - 1 / 2) * width * zoom_scale + x_off * zoom_scale;
      points[i].ypos =
        (random() - 1 / 2) * height * zoom_scale + y_off * zoom_scale;
      points[i].px = points[i].xpos;
      points[i].py = points[i].ypos;
      points[i].pdx = [];
      points[i].pdy = [];
